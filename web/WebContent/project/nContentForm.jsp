		...
		<table class="table">
			...
			<tr>
				...
				<td colspan="5"><!-- 온클릭시 아이피 받아서 넘어가게 ? -------------------------> 
					...
					<button  onclick = "window.location.href='nBookLikePro.jsp?num=<%=article.getNum()%>&pageNum=<%=pageNum%>'">좋아요!</button>
					<button  onclick = "window.location.href='nBookListForm.jsp?num=<%=article.getNum() %>&pageNum=<%=pageNum%>'">닫기</button>
					<%if(id!=null&&id.equals("admin")){ %>
					<button  onclick = "window.location.href='nBookdeletePro.jsp?num=<%=article.getNum() %>&pageNum=<%=pageNum%>&endRow=<%=endRow%>'">삭제</button>
					<%} %>
				</td>
			</tr>
		</table>
