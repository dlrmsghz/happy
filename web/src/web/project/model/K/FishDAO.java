...
public class FishDAO {
	...
	public void insertBook(FishDTO article) {
		...
		try {
			conn = getConnection();
			sql = "insert into nbook_list values (nbook_list_seq.nextVal,?,?,?,?,?,?,?,?,sysdate,0)";			
	...
	// 게시판 글 전체 개수 돌려주는 메서드
		...
		public List getArticle(int start, int end) {
			...
			try {
				...
				String sql = "select bk_num,bk_name,bk_img,bk_price,bk_genre,bk_writer,bk_publisher,bk_regs,bk_read_count,r "
						...
						+ "from nbook_list order by bk_num desc) order by bk_num desc)  "
						+ "where r >= ? and r <= ?";
				pstmt = conn.prepareStatement(sql);
		...
		public int getGenreArticleCount(String genre) {
			...
			// 책 리스트 에서 삭제
				...
				public void like(int num, String id) { // 좋아요
				...
				public int showLike(int num) { //좋아요 받은 숫자 보여주기
					int likefish = 0;
					try {
						conn = getConnection();					
				...
				public String getFishId(int num ,String id) { // 좋아요 이미 누른 아이디 확인
					String fishId = null;
					try {
						conn = getConnection();					
